...
void ColorManager::initColorValue(){
    ...
    firstTouchCrayonFine = false;
    firstTouchMarker = false;
    ...
    for (int i=0; i<30; i++) {
        ...
        colorCrayonFinePos[i] = 0.0;
        colorMarkerPos[i] = 0.0;
    }
}
...
Color3B ColorManager::returnPenColor(int _colorIndex) {
    return getCurColor(_colorIndex);
}
...
Texture2D* ColorManager::maskedWithSpriteTexture(Sprite* texturesp, Sprite* masksp,Point location)
{
...
void ColorManager::saveTextureToFile(std::string _fileName, ColorSprite *_colorSpr)
{
    ...
    RenderTexture* saverenderTexture = RenderTexture::create(sprSize.width, sprSize.height, kTexture2DPixelFormat_RGBA8888);
