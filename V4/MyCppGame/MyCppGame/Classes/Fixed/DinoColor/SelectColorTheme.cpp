...
//
            ...
            if (colorThemes!=NULL && colorThemes->getBoundingBox().containsPoint(location) && clickTheme==false && touchMoveDis==0  && touchMoveTheme==false) {
                ...
                if ( ColorManager::shared()->curColorTheme==i) {
                    ...
                    colorThemes->runAction(Sequence::createWithTwoActions(Sequence::createWithTwoActions(scaleBy, scaleBy->reverse()), CallFunc::create(CC_CALLBACK_0(SelectColorTheme::goNext, this))));
                }else{
                    touchMoveTheme = true;
                    clickTheme=false;
                    showCurTheme = true;
...
//                        CCLOG("---最后的位置%f---",ColorManager::shared()->colorThemePos[j]);
                        oldcolorThemes->runAction(Sequence::create(MoveTo::create(0.1, Vec2(oldcolorThemes->getPositionX()-moveIndex, winCenter.y)),
//                                                                     CallFunc::create(CC_CALLBACK_0(SelectColorTheme::resetTouchThemePos, this)),
//                                                                     CallFunc::create(CC_CALLBACK_0(SelectColorTheme::resetThemePos, this)),
                                                                     DelayTime::create(0.3),
